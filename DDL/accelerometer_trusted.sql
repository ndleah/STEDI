-- Create an external table named 'accelerometer_landing' in the database 'stedi-lakehouse'
-- If the table already exists, this command won't create a new one
CREATE EXTERNAL TABLE IF NOT EXISTS `stedi-data-lake`.`accelerometer_trusted` (
...
LOCATION 's3://stedi-data-lake/project/accelerometer/trusted/'
